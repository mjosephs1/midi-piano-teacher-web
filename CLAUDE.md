...
`AccidentalProvider` (`src/context/AccidentalContext.tsx`) holds the global `accidentalStyle` (`'sharp'` default) and persists it via `loadSettings`/`saveSettings` using a `settingsLoaded` state flag (instead of the usual ref) that also gates rendering: like `StorageProvider`, it returns `null` until the load resolves, so the app never paints the default `'sharp'` before the saved style arrives (no ♯→♭ flicker on reload). It sits inside `StorageProvider` since it needs `useStorage()`. The `♯/♭` toggle in the top nav (`App.tsx`, `.accidental-toggle`) shows the active symbol in black and the inactive one grayed out.
...
- **ChordExplorer** (`pages/ChordExplorer.tsx`) — chord-group dropdown + inversion dropdown + 12 root buttons on a 25-key piano. Base note is fixed at `KEYBOARD_OFFSETS[25]` (48/C3) regardless of the app's selected keyboard size. The Inversion dropdown offers 3 options (Root/1st/2nd) for triads or 4 (adds 3rd) for 7th-chord groups; switching chord groups resets the selected inversion to Root only if it would otherwise be out of range (e.g. 3rd inversion selected, switching to a triad group), otherwise the inversion selection is preserved. Voicing for a given inversion is computed locally via `invertIntervals()`, which rotates the chord group's `intervals` (moving tones below the inversion point up an octave) — not via `Chord`, since this page works directly with `CHORD_GROUPS` rather than `Chord` instances.
